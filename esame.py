...
class CSVTimeSeriesFile:
    ...
    def get_data(self):
        ...
        result = []
        #creo delle liste per salvare le date, per poi poter controllarle
        years = []
        months = []
        dates = []
        for line in openedfile:
            ...
            if (datas[0] != 'date'):
                ...
                #controllo che la data sia scritta correttamente
                year_month = datas[0].split("-")
                if (len(year_month) == 2 and year_month[0].isnumeric() == True and year_month[1].isnumeric() == True):
                    if (int(year_month[1]) >= 1 and int(year_month[1]) <= 12):
                        new.append(datas[0])
                        years.append(int(year_month[0]))
                        months.append(int(year_month[1]))
                        dates.append(datas[0])
                else:
                    print ("Errore riscontrato nella seguente data: {}".format(datas[0]))
                    error_found = True
                
                #controllo che il dato sia un numero intero positivo, altrimenti lo salto
                if (error_found == False):
                    if (((datas[1].strip()).replace("-","",1)).isnumeric() == True):
                        if (int(datas[1]) > 0):
...
def compute_avg_monthly_difference(time_series, first_year, last_year):
    ...
    result = []
    #creo un indice per iterare su i vari anni
    year_index = 0
    year_found = False
    while(year_found == False):
        ...
        if (first_year == year_searching[0]):
            for x in range(0,12):
                #salvo ogni risultato uguale a 0
                result.append(0.0)
